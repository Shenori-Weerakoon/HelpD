...
const SupportEngineerDashboard = () => {
    ...
    useEffect(() => {
        const fetchTickets = async () => {
            if (uid && role) {
                try {
                    console.log('Fetching tickets for UID:', uid, 'Role:', role);
                    const response = await axios.get(`http://localhost:5000/api/tickets?assignedTo=${uid}`);  // Pass role and uid
    ...
    const handleCardClick = async (statusType) => {
        try {
            const response = await axios.get(`http://localhost:5000/api/tickets?assignedTo=${uid}`); // Fetch all tickets again
            const tickets = response.data;
    ...
    return (
        <div className="h-screen flex">
            {/* Sidebar */}
            <Sidebar role="support_engineer" className="w-64 flex-shrink-0"/>
